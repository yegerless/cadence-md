...
Создайте `cadence-md-secret` вне git до применения workloads. Храните реальные
значения в secret manager или передавайте их напрямую в `kubectl` из безопасного
окружения.
...
Не коммитьте сгенерированные `Secret` manifests с реальными production values.
